...
object Graph {
  ...
  def traverse[A, B](tree: Tree[A])(convert: A => B): Seq[B] = {
    /*ListExtender adds the nodes to the list*/
    def ListExtender(elem: Tree[A], list: Seq[A]): Seq[A] = {
      elem match {
        case Node(x) => list.seq :+ x
        case Branch(left, right) =>
          ListExtender(left, ListExtender(right, list))
      }
    }
    ListExtender(tree, List()).reverse.map(convert)
  }
